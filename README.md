...
- `npm run build` - create the production bundle in `build/` (no prerender)
- `npm run build:prerender` - build, start preview, and prerender every localized route into `build/`. This is the real production build.
...
- `npm run verify:prod` - full production verification chain: `test` → `build:prerender` → `seo:validate`. Run this before any deploy.

Deploy:

- `npm run release:prod` - copy the verified `build/` artifact into `dist/release/site` with a release manifest
- `npm run deploy:prod:dry-run` - print the resolved FTP deploy config without uploading
- `npm run deploy:prod` - run `verify:prod` → `release:prod` → FTPS upload to Hetzner
- `npm run package:deploy` - generate `build-deploy.zip` for manual upload (fallback)

Misc:

- `npm run generate:og-image` - regenerate `public/og-image.png` (1200×630 social-sharing card)

> Note: `sitemap.xml` is generated by `scripts/generate-sitemap.mjs` during `npm run build:prerender` (not hand-edited). The `.htaccess` file owns server-side redirects including bare-alias 301s (`/about` → `/en/about/`).
